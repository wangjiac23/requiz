...
### V3.0.0 修订
1. v3.1.0 ① pi 按钮移至侧边栏头部（sidebarHead），面板可拖拽调宽（#piResizer，120~520px，拖到最窄隐藏）② pi 回复公式 KaTeX 渲染（sendPiMsg 后 renderMath）③ 设置面板 pi 配置（路径/模型存全局配置 `pi:` 节，`/api/config/global` 读写，pi.go 读配置执行）
